...
카드 하나의 굵기다. **항목별**이고, `calibration`(전역)과 다르다.

```json
"grain": {
  "size": "small",             // 시작 줄을 이 굵기로 쓴다
  "depth": "shallow|medium|deep",
  "level": "unfamiliar|familiar|unknown",
  "rationale": "왜 이 굵기인지. 내부 근거이고 사용자에게 쓰지 않는다",
  "on_stuck": {                // 크기가 이미 최소면 null
    "signal": "너무 큼을 누르거나 첫 동작이 안 떠오르면",
    "finer_size": "minimal"
  }
}
```

잘못 쪼갤 확률은 사람이 아니라 **사람 × 영역**에 붙는다. 그래서 영역별 기록(`domain_skill`)을 따로 둔다. 영역 키는 `context` 노드를 그대로 쓴다 — 이미 항목에 붙어 있고 사용자가 발화한 것에서만 온다. 영역을 따로 분류하지 않는 이유는 추론한 영역이 틀린 엣지와 같은 종류의 조용한 오염이 되기 때문이다.

| 신호 | 판정 | 굵기 |
|---|---|---|
| 이 맥락에서 `너무 큼`+`이탈` 2회 이상이고 완료 0 | `unfamiliar` | 한 단계 잘게 |
| 이 맥락에서 완료 3회 이상이고 `너무 큼` 0 | `familiar` | 한 단계 굵게 |
| 그 외 | `unknown` | 그대로 |

맥락이 여럿이면 **나쁜 쪽을 따른다.** 굵게 줬다가 못 하는 손해가 잘게 줬다가 지루한 손해보다 크다.

**캘린더 상한은 숙련도로 뚫지 못한다.** 시간이 없는 것은 실력과 무관하다.

**`단서 없음`(`no_cue`)은 숙련도에 세지 않는다.** 크기가 아니라 `시작` 줄이 틀렸다는 신호라서, 세면 엉뚱한 것을 고치게 된다. 미수행 원인 셋 중 분해 부족은 `too_big`, 시간 미스매치는 캘린더 `ceiling`, 단서 부재가 이것이다.
